...
export default class QlfApi {
  ...
  static async getProcessingHistory(start, end, order, offset) {
    ...
    try {
      const processes = await fetch(
        `${
          apiUrl
        }dashboard/api/processing_history/?format=json&limit=10&offset=${
          offset
        }&ordering=${order}&datemin=${start.split('T')[0]}&&datemax=${
          end.split('T')[0]
        }`,
        {
          method: 'GET',
  ...
  static async getObservingHistory(start, end, order, offset) {
    if (!start && !end) return;
    try {
      const exposures = await fetch(
        `${
          apiUrl
        }dashboard/api/observing_history/?format=json&limit=10&offset=${
          offset
        }&ordering=${order}&datemin=${start.split('T')[0]}&&datemax=${
          end.split('T')[0]
        }`,
