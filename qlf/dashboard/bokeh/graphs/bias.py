...
class Bias:
    def get_bias(selected_exposure, arm_select, spectrograph_select):
        data_model = {
            'bias': [],
        #    'bias_amp_1':[],
        #    'bias_amp_1_hist':[],
        ...
        exposure = list()
        for exp in get_exposure_ids():
            exposure.append(str(exp))
        ...
        if not exposure:
            exposure.append(selected_exposure)

        exp_select = Select(title="Exposure ID:", value=selected_exposure, options=exposure, width=100)
        ...
        def arm_handler(attr, old, value):
            update(value, spectrograph_select.value, exp_select.value)

        def spectrograph_handler(attr, old, value):
            update(arm_select.value, value, exp_select.value)
        ...
        exp_select.on_change("value", exposure_handler)
        ...
        update(arm_select.value, "3", selected_exposure)

        #plot = gridplot([[bias_plot]], responsive=True)
        # amp_plot = gridplot([[bias_amp1_plot,bias_amp2_plot],[bias_amp3_plot,bias_amp4_plot]])
        amp_plot = gridplot([[bias_amp1_plot,bias_amp2_plot],[bias_amp3_plot,bias_amp4_plot]])
        ...
        #         bias_plot, amp_plot)
        layout = column(bias_plot, amp_plot, responsive=True, css_classes=["bias_graph"])
